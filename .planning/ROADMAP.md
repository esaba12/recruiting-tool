...
- [x] 05-02-PLAN.md — Combined regression sweep + staged end-of-phase manual verification
...
- [x] 06-01-PLAN.md — Port Overview's 3 charts into TodayTab as an Activity Section, rendered outside the caught-up gate

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 06-02-PLAN.md — Atomic nav cutover: NAV_ITEMS 7→5, Settings footer + mobile affordances, retire OverviewTab, shrink DEMO_NAV_ITEMS

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 06-03-PLAN.md — NAV-03 deep-link relay audit + combined regression sweep + staged end-of-phase manual verification
...
- [x] 07-01-PLAN.md — Motion package migration (framer-motion → motion) + Modal/SidePanel shape system
- [x] 07-02-PLAN.md — Shape system: Card/Section/Button/Badge/Tabs/ChipToggleGroup
- [x] 07-03-PLAN.md — charts/theme.js resync + jobBoards/helpers.js bucket-color remap + RepoStats/UserProfileView shape
- [x] 07-04-PLAN.md — Mid-traffic color sweep: AI-fit buttons, Calendar/timeline badges, DuplicatesPanel, RepoJobsView WCAG fix, JobCard shape
- [x] 07-05-PLAN.md — Modal-shell + list-row shape sweep (AddEventModal, EventDetailModal, LogInteractionModal, KeepInTouchTab, TodayTab's OverdueRow)
- [x] 07-06-PLAN.md — Table/tooltip/dashboard shape sweep (ContactsTable, CalendarView, ChartTooltip, LoginPage, OutboxTab, AddToCalendarModal)
- [x] 07-07-PLAN.md — NetworkGraphTab wrapper shape + QuickCaptureModal chat bubbles + gradient-header sweep
...
- [x] 07-09-PLAN.md — Combined regression sweep + repo-wide audit closure + staged end-of-phase manual verification
...
## Milestone v1.1 — Recruiting Events (started 2026-09-13; executed directly, no GSD orchestration)
...
- [x] **Phase 9: Ingestion core + Localist adapter + dedup + staleness** — `lib/ingest/` lifted from `timelineFinder`/`discoveryScheduler`; WR-09 fixed; adapter interface; `api/events-ingest.js`; freshness gate → degraded sources.
- [x] **Phase 10: Attributes, registration deadlines, requirements, relevance** — once-per-event extraction; `lib/eventRelevance.js`; `attention.js` derivations.
- [x] **Phase 11: Calendar extension** — `updateEvent`, allowlist widening, dedicated Recruiting calendar (new scope + re-consent), sync semantics, CR-02.
- [x] **Phase 12: Conflict engine** — `lib/conflicts.js` free fragments, hard/soft blocks, all-day, straddle, tz, transit; tests first.
- [ ] **Phase 13: Paste-import, CSV/ICS upload, manual entry, moderation** — `EventImportModal` on the QuickCapture draft-card pattern; confidence gate; contributed dedup.
