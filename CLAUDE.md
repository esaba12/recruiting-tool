...
**"Sign in with API key" (`lib/apiKeyAuth.js`, shipped 2026-08-07):** a zero-signup fast path on `LoginPage.jsx` — paste an Anthropic key, get signed in (and auto-signed-up on first paste). The key is SHA-256 hashed client-side into a deterministic `key-<hash>@byok.local` email + hash-as-password pair, so the same key always resolves to the same account and Supabase Auth never sees the raw key; the raw key is separately saved as the account's Anthropic BYOK key via `/api/keys` (same encrypted-at-rest path everything else uses), so the app is immediately usable. **Deliberate tradeoff:** this makes the raw key double as the account's only credential, with no real email behind it — a leaked key now also grants CRM access, not just Anthropic spend, and rotating the key with nothing else set up permanently locks the account out (no reset-password path to a `@byok.local` address). Mitigated, not eliminated: `SettingsTab.jsx`'s "⚠ Secure your account" section (shown only when `user.email` ends in `@byok.local`) offers two independent recovery paths — link a Google identity (`linkGoogleIdentity()` in `lib/googleAuth.js`, via `supabase.auth.linkIdentity` — attaches to the *current* session rather than starting a new one, so it can't accidentally sign into/create a different account), or set a password (`supabase.auth.updateUser({ password })`, shown alongside the account's read-only `@byok.local` system email as the login identifier to pair it with). **Deliberately password-only, not email+password:** this project's `supabase/config.toml` has `double_confirm_changes = true`, which requires confirming an email change via both the old *and* new address — the old `@byok.local` address can't receive mail, so an email change would sit permanently pending. A password-only update has no such confirmation step and applies instantly. Not enforced or nagged on login; it's an opt-in warning banner, not a gate.
...
## API Hardening — rate limiting, security headers, path allowlisting (shipped 2026-08-10)

A security pass prompted by researching common "vibe-coded app" failure modes (missing rate limits, over-permissive proxies, missing security headers, unvalidated uploads — the five most common OWASP-flavored gaps in AI-assisted codebases per current industry write-ups). BYOK encryption, RLS, and `requireUser()` auth gates (see Multi-Tenant Auth + BYOK above) already covered the biggest risks; this closes the remaining gaps found by a targeted audit.

- **Per-user rate limiting (`api/_lib/rateLimit.js`)**: every proxy/CRUD endpoint (`claude-api`, `openai`, `exa`, `gh-api`, `google-calendar`, `keys`, `google-connect`) now calls `checkRateLimit(user.id, TIER)` right after `requireUser()` and 429s (with `Retry-After`) if exceeded. Backed by Upstash Redis (`@upstash/ratelimit` sliding window), provisioned via `vercel integration add upstash` → "Upstash for Redis". **Reads `KV_REST_API_URL`/`KV_REST_API_TOKEN`** (falls back to raw `UPSTASH_REDIS_REST_URL`/`TOKEN` if ever present) — the Marketplace integration injects the legacy `@vercel/kv`-compatible names, not the raw Upstash ones its own docs show; confirmed against this project's actual `vercel env ls` output. **Fails open (no-op) if neither pair is set**, so local dev and any environment that hasn't provisioned Redis yet still work; this is a defense-in-depth layer against cost/abuse (a leaked BYOK key or a client-side retry-loop bug burning unbounded Anthropic/OpenAI/Exa spend), not an auth gate. Tiers: `AI`/`SEARCH` 30 req/60s, `GITHUB`/`CALENDAR` 60 req/60s, `CRUD` (key management) 20 req/60s.
  - **Provisioned and verified live in production 2026-08-11**: signed up a disposable test account (`enable_confirmations = false` in `supabase/config.toml` means no email-click needed) against `https://recruiting-os-phi.vercel.app`, hammered `/exa/search` with 40 concurrent requests — got 28×400 (correctly past the limiter, failing only on missing BYOK key) + 12×429 with `Retry-After` populated, exactly matching the 30/60s `SEARCH` tier. Rate limiting is live, not just wired.
  - Local dev still needs the two vars copied from `app/.env.local` (written by `vercel env pull`) into the root `.env` that `vite.config.js` actually reads from — Claude Code's own permission settings block reading/writing `.env*` files directly, so this one step has to be done by hand: `cd app && grep -E '^(KV_REST_API_URL|KV_REST_API_TOKEN)=' .env.local >> ../.env`.
- **Path allowlisting** on the 5 `?path=`-passthrough proxies (`claude-api`, `openai`, `exa`, `gh-api`, `google-calendar`) — previously any authenticated user could hit *any* endpoint on api.anthropic.com/api.openai.com/api.github.com/googleapis.com through the app, not just the intended ones (host couldn't be hijacked since it's string-concatenated not resolved from `path`, but there was no defense-in-depth). Each proxy now 403s any `path` outside its actual usage: `claude-api` → `v1/messages` only; `openai` → `v1/chat/completions` only; `exa` → `search`/`findSimilar`/`contents` only; `gh-api` → `repos/*`/`users/*`/`orgs/*` only; `google-calendar` → `calendar/v3/calendars/primary/events*` only. **Path check runs before the BYOK-key/refresh-token lookup** in every proxy (not after, as first written) — fails fast on a bad path without an unnecessary DB round-trip, and matters for testability too: with the check after the key lookup, a user with no BYOK key got the same 400 for both allowed and disallowed paths, masking whether the allowlist was doing anything. Verified live in production: bad paths on `exa`/`claude-api`/`gh-api` all 403, `gh-api/repos/vercel/next.js` (allowed shape) returned a real 200 from GitHub.
- **Security headers (`vercel.json`)**: added a `headers` block — `Content-Security-Policy` (default-src 'self', connect-src allows `https://*.supabase.co`, frame-ancestors 'none'), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`. Closes a clickjacking gap (app could previously be iframed and UI-redressed into triggering real actions — delete BYOK key, create calendar event, etc.). Vercel-only (`vercel.json` isn't read by the Vite dev server), so local dev is unaffected.
- **`AddToCalendarModal.jsx` upload guard**: `accept="image/*"` was UI-only (no server- or client-side enforcement) and `downscaleImage()`'s promise rejection was unhandled. Now checks `file.type`/`file.size` (20MB ceiling) before decode and catches decode failures into the existing error banner — low severity (the canvas re-encode already bounded what actually reached Claude vision), but removes an unhandled-rejection footgun.
- **Audited and confirmed already fine, no changes needed**: BYOK keys/service-role key never reach the client bundle; `user_api_keys`/`google_calendar_tokens` RLS is correctly default-deny (zero policies); all DB access goes through supabase-js's query builder (no raw SQL / injection surface); no `dangerouslySetInnerHTML` or unsafe `innerHTML` anywhere in `app/src`; CSRF isn't a meaningful risk (Bearer-JWT auth, not cookies); Google OAuth state/PKCE is handled by `supabase-js` itself, not custom code; no secrets ever hit `console.log`/`console.error` in `api/*.js`; `npm audit --production` is clean (0 vulnerabilities — the 4 found by a full `npm audit` are all pre-existing **dev-only** tooling, `vite`/`esbuild`/`postcss`/`nanoid`, not shipped to production or the client bundle, and not introduced by this pass).
...
### Multi-calendar: Personal + School (shipped 2026-08-10)

Each user can connect **two independent Google accounts** — a `personal` slot and a `school` slot (`api/_lib/googleOAuth.js`'s `CALENDAR_SLOTS`) — not just one. `google_calendar_tokens` went from `user_id` as its primary key to a composite `(user_id, slot)` (migration `20260810000000_multi_calendar_slots.sql`; existing single-connection rows became `slot='personal'` via the column default, so nobody who'd already connected needed to reconnect). Every read/write path threads a `slot` query param through: `api/google-calendar.js` picks which stored refresh token to mint an access token from, `googleCalendar.js`'s `listEvents()` fetches both slots in parallel via `Promise.allSettled` and merges (tagging each normalized event with `.slot`, failing soft per-slot so one disconnected/expired calendar never hides the other's events), and `createEvent`/`deleteEvent` take an explicit `slot`. `AddEventModal.jsx` and `AddToCalendarModal.jsx` both got a Personal/School picker on their create step; `CalendarTab.jsx`'s day-agenda rows and `EventDetailModal.jsx` show which calendar an event belongs to (accent = personal, purple = school); `SettingsTab.jsx`'s Google Calendar section is two independent connect/disconnect rows instead of one.

**Why not just re-run the existing "Connect Calendar" flow twice?** The original flow piggybacked on `supabase.auth.signInWithOAuth({ provider: 'google' })` to get Google's consent screen for free, since Supabase already had a configured Google OAuth client. That's fine for exactly one Google account, but it's unsound for a second, *different* account: `signInWithOAuth` manages the app's own login identity, so picking a different Google account there risks switching (or creating) the signed-in Supabase user rather than just adding calendar access — and `linkIdentity` (the actually-safe alternative, used by `linkGoogleIdentity()` for BYOK account recovery) only supports one linked identity per provider per user, so it can't hold two distinct Google accounts either. Instead, connecting a calendar now goes through a **direct Google OAuth flow that never touches Supabase Auth**: `lib/googleAuth.js`'s `connectGoogleCalendar(slot)` calls `api/google-oauth-start.js` (authenticated; mints an AES-256-GCM-encrypted `state` binding the request to `{userId, slot, exp}`, same cipher `_lib/crypto.js` already uses for BYOK keys) → full-page redirect to Google → `api/google-oauth-callback.js` (a plain unauthenticated GET, since it's Google's redirect target; trusts the encrypted `state` instead) exchanges the code, fetches the connected email, and upserts the token under that slot. Same `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` as before — just a second, independent redirect URI (`/api/google-oauth-callback`) registered on that same OAuth client, not a new Cloud project. The old `finishGoogleCalendarConnect()` session-capture mechanism and its `POST /api/google-connect` are retired; `api/google-connect.js` is now status/disconnect only (`GET`/`DELETE`, both `?slot=`-aware).

**Manual one-time setup required (not yet done as of 2026-08-10):**
...
- **Pipeline** — Application tracker from Applications DB, stage funnel, `DuplicatesPanel` (see below). Each application can record who referred you for it (`referred_by_id`, linked to a Contact) — editable manually in the UI, and since 2026-08-10 also auto-set by the email pipeline when an email explicitly names a referrer and it resolves to an existing Contact (see Email Pipeline); never overwritten once set, so a manual correction always sticks.
...
Deploy: script.google.com → paste file → Script Properties → add `ANTHROPIC_API_KEY` + `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` + `RECRUITING_USER_ID` → run `setup()` → add trigger (every 10 min)

**Discovery (fixed 2026-08-10 — see below) → Claude Haiku classifies + extracts (newest message only) → upsert contact in Supabase (`contacts` table) → upsert application (`applications` table) → Google Calendar event (if interview date found) → every message new since the last run (tracked per-thread via `PropertiesService`, key `msgcount_<threadId>`) gets logged as its own row in `interactions` (no LLM call) → `recruiting-done` label applied as a visual marker only (no longer gates processing, since a thread can grow replies after being marked done).**

**Discovery is three Gmail searches, deduped by thread ID, not just the manually-applied `recruiting` label:** (1) threads already labeled `recruiting`, (2) recent Sent-folder threads not yet labeled (`SENT_SCAN_QUERY`, catches the user's own new cold outreach), and (3) recent Inbox threads not yet labeled that look recruiting-shaped by sender/subject (`INBOX_SCAN_QUERY` — a known ATS domain like `greenhouse.io`/`myworkday.com`/`lever.co`, or a subject containing "application"/"interview"/"offer"/etc.). Source (3) is the fix for the original bug: an inbound "thanks for applying" confirmation that was never manually labeled used to be completely invisible to the script. The keyword list is intentionally broad recall, not a precision filter — Claude's `UNRELATED` classification is the real filter, so a false-positive Gmail match just costs one cheap Haiku call, while a false negative was a silently-missed application.

**Classification recognizes application confirmations, not just replies/invites/offers/rejections:** `type` includes `APPLICATION_CONFIRMATION` (an automated "we received your application" ATS/no-reply acknowledgment, before any human has replied) alongside `REPLY|INTERVIEW_INVITE|OFFER|REJECTION|NEW_CONTACT|FOLLOW_UP_NEEDED`, and `upsertApplication()` now fires on `APPLICATION_CONFIRMATION` too (creating the Pipeline row at stage `Applied` the moment the application lands, not only once a human responds). Cheap deterministic regex hints (`APPLICATION_CONFIRMATION_RE`, a sender-domain company guess via `guessCompanyHint()`) are fed into the Claude prompt the same way the calendar-invite/meeting-link hints already were, to raise recall on a single-shot Haiku call. A `STAGE_RANK` guard on `upsertApplication()` stops a later, fuzzy-company-matched signal from regressing an application's stage backward (e.g. a second, unrelated confirmation email can't knock a `Phone Screen` back down to `Applied`) — `REJECTION` is exempt since it's terminal and can land at any stage.

**Referrer extraction:** the prompt now also asks for `referrer_name` — the name of whoever referred/recommended the candidate, extracted *only* when the email explicitly says so (e.g. "referred by Jane Doe", "submitted your referral"), with a regex hint (`REFERRAL_MENTION_RE`) raising recall the same way. `upsertApplication()` resolves that name to an existing Contact by fuzzy name match (`findContactByName()`) and sets `applications.referred_by_id` — but only if the application doesn't already have one, so a later email can never silently overwrite a manually-corrected referrer. This reverses the prior behavior/limitation documented below.

**Ported to Supabase (2026-08-01)** — writes directly to this app's live Postgres tables via raw PostgREST calls (`UrlFetchApp`, no npm available in Apps Script) using the service-role key, the same server-side trust model `api/_lib/supabaseAdmin.js` uses for the Vercel proxies. Since Apps Script has no session to derive a user from (unlike every other server-side call site, which uses `requireUser(req)`), `RECRUITING_USER_ID` is a single hardcoded user id included explicitly on every query/insert — this script serves exactly one recruiting-os account, not a multi-tenant signup flow. One deliberate behavior change from the old Notion version: contact `status` is only set on **create**, not overwritten on every re-processed email, so an automated run can no longer silently clobber a status you changed manually in the UI.
...
Not yet built: mobile quick-capture generalized across both action modals (the "+ Event" floating button pattern now exists in `Sidebar.jsx` — extending the same affordance to `LogInteractionModal` is now cheap), weekly stale-contact digest (Apps Script time trigger), "looks cold" decay badge.
