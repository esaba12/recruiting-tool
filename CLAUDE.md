...
- **Pipeline** — Application tracker from Applications DB, stage funnel, `DuplicatesPanel` (see below). Each application can record who referred you for it (`referred_by_id`, linked to a Contact) — manual only, since email content never carries that signal (see Email Pipeline).
...
Pipeline: Gmail label `recruiting` → Claude Haiku classifies + extracts (newest message only) → upsert contact in Supabase (`contacts` table) → upsert application (`applications` table) → Google Calendar event (if interview date found) → every message new since the last run (tracked per-thread via `PropertiesService`, key `msgcount_<threadId>`) gets logged as its own row in `interactions` (no LLM call) → `recruiting-done` label applied as a visual marker only (no longer gates processing, since a thread can grow replies after being marked done)

**Ported to Supabase (2026-08-01)** — writes directly to this app's live Postgres tables via raw PostgREST calls (`UrlFetchApp`, no npm available in Apps Script) using the service-role key, the same server-side trust model `api/_lib/supabaseAdmin.js` uses for the Vercel proxies. Since Apps Script has no session to derive a user from (unlike every other server-side call site, which uses `requireUser(req)`), `RECRUITING_USER_ID` is a single hardcoded user id included explicitly on every query/insert — this script serves exactly one recruiting-os account, not a multi-tenant signup flow. One deliberate behavior change from the old Notion version: contact `status` is only set on **create**, not overwritten on every re-processed email, so an automated run can no longer silently clobber a status you changed manually in the UI. `applications.referred_by_id` (see Pipeline tab below) is never touched by this pipeline — no email content carries a "who referred me" signal, so it stays a manual field.
