    ...
    <>
      ...
      <aside className="hidden md:flex flex-col w-60 shrink-0 bg-ink-900 text-ink-50 h-screen sticky top-0 overflow-y-auto">
        ...
        <div className="px-3 py-4 border-t border-ink-800 space-y-2">
          ...
          {demoMode ? (
            <a href="/" className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-ink-800 text-ink-100 hover:bg-ink-700 transition-colors">
              Sign up free →
            </a>
          ) : (
          ...
          <p className="text-[11px] text-ink-500 mt-2 text-center">
            {demoMode ? 'Changes reset on reload' : lastLoaded ? `Updated ${lastLoaded}` : 'Loading...'}
          </p>
        </div>
      </aside>
