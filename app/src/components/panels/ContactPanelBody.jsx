// Ported from ../ContactDetailModal.jsx (Phase 4, Plan 04-02).
// Behavioral additions: an optional `onBack` header affordance, and the interaction-logging dialog now renders through a portal to escape the panel's transform.
          ...
          <div className="flex items-center gap-2 min-w-0">
            {onBack && (
              <button onClick={onBack} aria-label="Back"
                className="shrink-0 w-7 h-7 rounded-full bg-ink-100 hover:bg-ink-200 flex items-center justify-center text-ink-500">
                <ArrowLeft size={14} />
              </button>
            )}
            <h2 className="text-base font-bold text-ink-900 truncate">{isNew ? 'New Contact' : contact.name}</h2>
          </div>
